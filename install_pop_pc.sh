...
rm -f pop pop-v0.3.1-linux-x64.tar.gz pop-v0.3.2-linux-x64.tar.gz
...
docker build --no-cache -t popnode .
docker run -d \
  --name popnode \
  -p 80:80 \
  -p 443:443 \
...
echo ""
echo -e "${BOLD}${CYAN}✅ PoP Node v0.3.2 installed & running inside Docker!${RESET}"
